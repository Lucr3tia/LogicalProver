...
class Prover:
    ...
    def print_proof_tree(self, sequent, depth=0):
        ...
        indent = "|" + "---" * depth  # Отступ для текущего уровня
        print(f"{indent}Глубина: {sequent.depth}. Секвент: {sequent}")
            ...
            while True:
                ...
                if apply_left:
                    if isinstance(left_expression, Negation):
                        indent = "|" + "---" * old_sequent.depth
                        print(f"{indent}Перебрасываем левую часть {left_expression} в правую:")
                    ...
                    if isinstance(left_expression, Implication):
                        indent = "|" + "---" * old_sequent.depth
                        print(f"{indent}Применение modus ponens к выражению {left_expression}:")
                ...
                if apply_right:
                    if isinstance(right_expression, Negation):
                        indent = "|" + "---" * old_sequent.depth
                        print(f"{indent}Перебрасываем правую часть {right_expression} в левую:")
                        new_sequent = remove_right_negation(old_sequent, right_expression)
                        frontier.append(new_sequent)  # Добавляем новый секвент в frontier
                        break
                    if isinstance(right_expression, Implication):
                        indent = "|" + "---" * old_sequent.depth
                        print(f"{indent}Применяем теорему о дедукции к выражению {right_expression}:")
